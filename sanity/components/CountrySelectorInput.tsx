...
export default function CountrySelectorInput(props: any) {
  ...
  return (
    <div style={{ padding: "0.5rem 0" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
        {visible.map((country) => {
          const checked = value.includes(country.value)

          return (
            <label key={country.value} style={{ cursor: "pointer" }}>
              <input
                type="checkbox"
                checked={checked}
                onChange={(e) => {
                  ...
                  onChange(
                    PatchEvent.from(
                      newValue.length ? set(newValue) : unset()
                    )
                  )
                }}
              />
